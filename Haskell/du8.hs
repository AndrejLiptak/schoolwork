...
cmpHanged wanted (Alive i) = compare wanted (fromIntegral i)
cmpHanged wanted (Grave left right) = if wanted >= left && wanted <= right then EQ
                                      else if wanted > right then GT
                                      else LT

-- | Check if a value is alive in a (correct) 'HTree'.
isAlive :: Integer -> HTree -> Bool
isAlive wanted (Node (Alive i) binl binr) = if com wanted (Alive i) == EQ then True
                                            else if com wanted (Alive i) == LT then isAlive wanted binl
                                            else isAlive wanted binr
                                            where com w i = cmpHanged (fromIntegral w) i
isAlive wanted (Node (Grave left right) binl binr) =  if com wanted (Grave left right) == EQ then False
                                                      else if com wanted (Grave left right) == LT then isAlive wanted binl
                                                      else isAlive wanted binr
                                                      where com w i = cmpHanged (fromIntegral w) i
...
-- numbers from 'ZEx'.
isGraveCor :: Hanged -> Bool
isGraveCor (Grave NegInf PosInf) = True
isGraveCor (Grave NegInf NegInf) = False
isGraveCor (Grave PosInf PosInf) = False
isGraveCor (Grave PosInf NegInf) = False
isGraveCor (Grave a b) = a <= b



isSuccHanged :: Hanged -> Hanged -> Bool
isSuccHanged (Alive first) (Alive second) = first + 1 == second
isSuccHanged (Alive i) (Grave l r) = fromIntegral i + 1 == l && isGraveCor (Grave l r)
isSuccHanged (Grave l r) (Alive i) = r + 1 == fromIntegral i && isGraveCor (Grave l r)
isSuccHanged (Grave l1 r1) (Grave l2 r2) = r1 + 1 == l2 && isGraveCor (Grave l1 r1) && isGraveCor(Grave l2 r2)

isOrdered :: [Hanged] -> Bool
isOrdered [] = True
isOrdered (x:[]) = True
isOrdered (x:y:xs) = isSuccHanged x y && isOrdered (y:xs)

isLast :: Hanged -> Bool
isLast (Grave  _ PosInf) = True
isLast _ = False

isFirst :: Hanged -> Bool
isFirst (Grave NegInf _) = True
isFirst _ = False

containsEveryNumber :: [Hanged] -> Bool
containsEveryNumber s = isLast (s !! ((length s ) - 1)) && (isFirst (s !! 0))


isCor :: [Hanged] -> Bool
isCor [] = False
isCor ((Grave NegInf PosInf):[]) = True
isCor ((Grave NegInf PosInf):xs) = False
isCor (x:[]) = False
isCor s = isOrdered s && containsEveryNumber s

isCorrect :: HTree -> Bool
isCorrect tree = isCor (inorder tree)
...
livingMembers tree =   map getI $ filter isAl $ inorder tree
                      where isAl (Alive _) = True
                            isAl _ = False
                            getI (Alive i) = i
...
moveBy (Node (Alive i) left right) k = Node (Alive (i+k)) (moveBy left k) (moveBy right k)
moveBy (Node (Grave l r) left right) k = Node (Grave (l+fromIntegral k) (r+ fromIntegral k)) (moveBy left k) (moveBy right k)
...
mirror :: HTree -> HTree
mirror Empty = Empty
mirror (Node (Alive i) left right) = Node (Alive (-1*i)) (mirror right) (mirror left)
mirror (Node (Grave l r) left right) = Node (Grave (-1*r) (-1*l)) (mirror right) (mirror left)
